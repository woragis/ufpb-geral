...
export const enrichCalculoVetorialLatex: DomainLatexEnricher = {
  ...
  respostaFinal(problem, solution) {
    ...
    switch (d.tipo) {
      ...
      case "produto-vetorial": {
        const [u1, u2, u3] = dados<ProdutoVetorialData>(problem).u;
        const [v1, v2, v3] = dados<ProdutoVetorialData>(problem).v;
        const i = u2 * v3 - u3 * v2;
        const j = -(u1 * v3 - u3 * v1);
        const k = u1 * v2 - u2 * v1;
  ...
  stepCalculo(problem, step) {
    ...
    switch (d.tipo) {
      ...
      case "campos": {
        const x = dados<CamposData>(problem);
        if (step.ordem === 1) {
          return `${partial("x^2 + y^2", "x")} = 2x,\\quad ${partial("x^2 + y^2", "y")} = 2y`;
        }
        if (step.ordem === 2) {
